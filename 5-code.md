...
- [Технические детали](#технические-детали)
  - [F-H-1. SSRF при создании продукта (High)](#f-h-1-ssrf-при-создании-продукта-high)
  - [F-H-2. Использование слабого секрета JWT (High)](#f-h-2-использование-слабого-секрета-jwt-high)
  - [F-M-3. Плохое хеширование паролей (Medium)](#f-m-3-плохое-хеширование-паролей-medium)
  - [F-M-4. Обход пути при загрузке файла (Medium)](#f-m-4-обход-пути-при-загрузке-файла-medium)
  - [F-H-5. Обход пути при чтении файлов (Medium)](#f-h-5-обход-пути-при-чтении-файлов-medium)
  - [F-M-6. Изменение пароля без подтверждения текущего (Medium)](#f-m-6-изменение-пароля-без-подтверждения-текущего-medium)
  - [F-L-7. Недостаточные требования к паролям (Low)](#f-l-7-недостаточные-требования-к-паролям-low)
  - [F-L-8. JWT не аннулируются при выходе (Low)](#f-l-8-jwt-не-аннулируются-при-выходе-low)
  - [F-L-9. Небезопасные атрибуты Cookie (Low)](#f-l-9-небезопасные-атрибуты-cookie-low)
  - [F-L-10. Открытое перенаправление (Low)](#f-l-10-открытое-перенаправление-low)
  - [F-L-11. Доступ к потенциальным техническим ресурсам (Low)](#f-l-11-доступ-к-потенциальным-техническим-ресурсам-low)
- [Цепочки эксплуатации](#цепочки-эксплуатации)
- [Невошедшее](#Невошедшее)
  - [F-M-11. Внедрение CRLF (Medium)](#f-m-11-внедрение-crlf-medium)
...
# Невошедшее
*Всё что будет описано дальше не включено в основной перечень, так как было найдено после отправки на проверку. И думаю, добавление в отдельный пункт это будет честно.*
## F-M-11. Внедрение CRLF (Medium)
### Классификация
- CWE-93: Improper Neutralization of CRLF Sequences ('CRLF Injection')
- OWASP: A05:2025 Injection
### Описание
Неправильная обработка последовательностей CRLF влечет за собой угрозы эксплуатации XSS и CSRF.
### Риск
- Подмена тела ответа -> XSS, HTMLi
- Внедрение заголовков
- Web Cache Poisoning
- Обход ограничений SameSite cookie
- Подмена Content-Security-Policy
### Затронутая область
Конечная точка: `/go` (GET)
...
class ServiceMiddleware:
    ...
    def __call__(self, environ, start_response):
        ...
        if path == "/go":
            ...
            target = urllib.parse.parse_qs(qs).get("url", ["/"])[0]
            ##
            # Декодирование пользовательского ввода без какой-либо фильтрации.
            # %0d%0a -> \r\n
            ##
            target = urllib.parse.unquote(target)
            ##
            # Значение target используется без какой-либо проверки на схожесть с URL.
            # Также, используется низкоуровневая функция start_response.
            ##
            start_response("302 Found", [
                ("Location", target),
                ("Content-Length", "0"),
...
```
### PoC
- Реализация подмены тела ответа с внедрением HTML и выполнением скрипта
    ```
    http://127.0.0.1:5000/go?url=%0D%0AContent-Type:%20text/html;%20charset=utf-8%0D%0A%0D%0A<img%20src=x onerror=alert(1)><!--
    ```
### Исправление
- Разрешать только относительные пути внутри приложения.
- Белый список разрешенных доменов (если нужны внешние перенаправления).
- Санитизация (удалять CRLF).
- Использовать проверенные библиотеки с похожим функционалом (например `redirect` в Flask).
